...
program_to_jinja={
    ...
    "children list":"('\n - '~(vessel.children|map(attribute='full_name_with_id')|join('\n - ') ) ) if vessel.children",
    "paradise paradoxes":"('\n - '~(atlas|map(attribute='full_name_with_id')|join('\n - ') ) ) if atlas",
    "paradise spells":"('\n - '~(spells|map(attribute='full_name_with_id')|join('\n - ') ) ) if spells",
    "paradise tunnels":"('\n - '~(tunnels|map(attribute='full_name_with_id')|join('\n - ') ) ) if tunnels",
    "paradise count":"universe|length",
    "siblings count":"vessel.siblings|length",
    "siblings random":"(vessel.siblings|random).name",
    "siblings list":"('\n - '~(vessel.siblings|map(attribute='full_name_with_id')|join('\n - ') ) ) if vessel.siblings",
    "time day":"time.day",
    "time year":"time.year",
